...
class StockModel extends Model
{
    ...
    public function getFilterSisaPakaiSpandex($bulan = null, $noModel = null, $kodeWarna = null)
    {
        $builder = $this->select('
        stock.no_model, stock.item_type, stock.kode_warna, stock.warna, stock.kgs_stock_awal, stock.lot_awal,
        mo.lco_date, mo.foll_up, mo.no_order, mo.buyer, mo.delivery_awal, mo.delivery_akhir, mo.unit,
    ...
    public function getFilterSisaPakaiKaret($bulan = null, $noModel = null, $kodeWarna = null)
    {
        $builder = $this->select('
        stock.no_model, stock.item_type, stock.kode_warna, stock.warna, stock.kgs_stock_awal, stock.lot_awal,
        mo.lco_date, mo.foll_up, mo.no_order, mo.buyer, mo.delivery_awal, mo.delivery_akhir, mo.unit,
    ...
    public function searchStockDetail($noModel = null, $warna = null)
    {
        ...
        st.nama_cluster,
        cl.kapasitas,
        COALESCE(hs.kgs, 0) AS KgsStockAwal,
        COALESCE(hs.krg, 0) AS KrgStockAwal,
        COALESCE(hs.cns, 0) AS CnsStockAwal,
        hs.lot AS lot_awal,
        oc.no_karung,
        (COALESCE(oc.kgs_kirim, 0)
            - COALESCE(oo.kgs_other_out, 0)
            - COALESCE(p.kgs_out, 0)) AS Kgs,
        (COALESCE(oc.cones_kirim, 0)
            - COALESCE(oo.cns_other_out, 0)
            - COALESCE(p.cns_out, 0)) AS Cns,
        oc.lot_kirim AS lot_stock
    ");

        // join tabel-tabel terkait
        $builder->join('pemasukan pm', 'pm.id_stock = st.id_stock', 'left');
        $builder->join('cluster cl', 'cl.nama_cluster = st.nama_cluster', 'left');
        $builder->join('out_celup oc', 'oc.id_out_celup = pm.id_out_celup', 'left');
        $builder->join('other_out oo', 'oo.id_out_celup = oc.id_out_celup', 'left');
        $builder->join('pengeluaran p', 'p.id_out_celup = oc.id_out_celup', 'left');

        // subquery untuk history_stock
        $builder->join('(
        SELECT 
            id_out_celup, 
            id_stock_new, 
            ...
            lot
        FROM history_stock
        GROUP BY id_out_celup, id_stock_new
    ) hs', 'hs.id_out_celup = oc.id_out_celup AND hs.id_stock_new = st.id_stock', 'left');

        // kondisi dinamis
        if (!empty($noModel)) {
            $builder->groupStart()
                ->like('st.no_model', $noModel)
                ->orLike('cl.nama_cluster', $noModel)
                ->groupEnd();
        }

        if (!empty($warna)) {
            $builder->like('st.kode_warna', $warna);
        }

        // Hanya ambil data dengan stok awal atau stok in/out ≠ 0
        $builder->groupStart()
            ->where('st.kgs_stock_awal <>', 0)
            ->orWhere('st.kgs_in_out <>', 0)
            ->groupEnd();
