...
class StockModel extends Model
{
    ...
    public function listCluster(string $noModel, string $itemType, string $kodeWarna, string $warna): array
    {
        return $this->select("
                ...
                SUM(COALESCE(krg_stock_awal,0) + COALESCE(krg_in_out,0)) AS total_krg,
                CASE 
                    WHEN lot_awal IS NOT NULL AND lot_awal <> '' THEN lot_awal
                    ELSE lot_stock
                END AS lot_final
            ")
            ->where([
    ...
    ')
            ...
            ->join('pengeluaran', 'pengeluaran.id_stock = stock.id_stock', 'left')
            ->join('material', 'material.id_order = mo.id_order AND material.item_type = stock.item_type AND material.kode_warna = stock.kode_warna', 'left')
        ...
        if (!empty($kodeWarna)) {
            $builder->like('stock.kode_warna', $kodeWarna);
        }
