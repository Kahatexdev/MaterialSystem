...
class WarehouseController extends BaseController
{
        ...
        if ($cekDuplikat == 0) {
            ...
            if ($this->pemasukanModel->insertBatch($dataPemasukan)) {
                ...
                foreach ($checkedIds as $key => $idOut) {
                    ...
                    // Override no_model kalau cluster == STOCK
                    $noModel = ($namaClusters == 'STOCK') ? 'BENANG STOCK' : ($noModels[$key] ?? null);
    ...
    public function savePengeluaranSelainOrder()
    {
        ...
        try {
            ...
            foreach ($items as $row) {
                ...
                // ===== VALIDASI ITEM =====
                if (empty($row['id_out_celup'])) {
                    throw new \Exception('ID Out Celup tidak boleh kosong');
                }
                ...
                $cnsKeluar = (int) (
                    ...
                    +
                    ($this->otherOutModel
                        ->selectSum('cns_other_out')
                        ->where('id_out_celup', $row['id_out_celup'])
                        ->first()['cns_other_out'] ?? 0)
                );

                $oc = $this->outCelupModel->find($row['id_out_celup']);

                if (!isset($row['kgs']) || floatval($row['kgs']) <= 0) {
                    $row['kgs'] = $oc['kgs_kirim'] - $kgsKeluar;
                }

                if (!isset($row['cns']) || intval($row['cns']) <= 0) {
                    $row['cns'] = $oc['cones_kirim'] - $cnsKeluar;
                }

                $rowKgs = (float)$row['kgs'];
                $rowCns = (int)$row['cns'];

                // BARU hitung krg
                $kgsKeluarReal = $kgsKeluar + $rowKgs;

                $row['krg'] = round($kgsMasuk, 2) === round($kgsKeluarReal, 2) ? 1 : 0;

                if (round($kgsMasuk, 2) === round($kgsKeluarReal, 2)) {
                    $this->pemasukanModel
                        ->where('id_out_celup', $row['id_out_celup'])
                        ->where('id_stock', $idStock)
                        ->set('out_jalur', '1')
                ...
                $rowKgs = (float) ($row['kgs'] ?? 0);
                $rowCns = (int)   ($row['cns'] ?? 0);
                $rowKrg = (int)   ($row['krg'] ?? 0);

                $stockKgsAwal = (float) $stock['kgs_stock_awal'];
                $stockCnsAwal = (int)   $stock['cns_stock_awal'];
                $stockKrgAwal = (int)   $stock['krg_stock_awal'];

                $stockKgsInOut = (float) $stock['kgs_in_out'];
                $stockCnsInOut = (int)   $stock['cns_in_out'];
                $stockKrgInOut = (int)   $stock['krg_in_out'];

                if ($stockKgsAwal >= $rowKgs) {
                    $kgsNew = $stockKgsAwal - $rowKgs;
                    $cnsNew = $stockCnsAwal - $rowCns;
                    $krgNew = $stockKrgAwal - $rowKrg;
                    $this->stockModel->set('kgs_stock_awal', $kgsNew)
                        ->set('cns_stock_awal', $cnsNew)
                        ->set('krg_stock_awal', $krgNew)
                        ->where('id_stock', $idStock)
                        ->update();
                } elseif ($stockKgsInOut >= $rowKgs) {
                    $kgsNew = $stockKgsInOut - $rowKgs;
                    $cnsNew = $stockCnsInOut - $rowCns;
                    $krgNew = $stockKrgInOut - $rowKrg;
                    $this->stockModel->set('kgs_in_out', $kgsNew)
                        ->set('cns_in_out', $cnsNew)
                        ->set('krg_in_out', $krgNew)
                        ->where('id_stock', $idStock)
                ...
                // ===== INSERT OTHER OUT =====
                $this->otherOutModel->insert([
                    'id_out_celup'  => $row['id_out_celup'],
                    'kategori'      => $kategori,
                    'tgl_other_out' => $tglOut,
                    'keterangan'    => $keterangan,
                    'kgs_other_out' => $row['kgs'],
                    'cns_other_out' => $row['cns'] ?? 0,
                    'krg_other_out' => $row['krg'] ?? 0,
                    'lot_other_out' => $row['lot'] ?? null,
                    'admin'         => session('username'),
                    'created_at'    => date('Y-m-d H:i:s'),
                    'nama_cluster'  => $stock['nama_cluster'],
                ]);
            }
            ...
            log_message('error', '[savePengeluaranSelainOrder] ' . $e->getMessage());
    ...
    public function getNoKarung()
    {
        $noModel   = $this->request->getGet('noModel');
        $itemType  = $this->request->getGet('itemType');
        $kodeWarna = $this->request->getGet('kodeWarna');
        $lot       = $this->request->getGet('lot');
        $retur     = $this->request->getGet('retur'); // 1 atau 0
        ...
        // Ambil da ta berdasarkan no_model yang dipilih
        if ($retur == 0) {
            $data = $this->outCelupModel->getNoKarung($id);
        } else {
    ...
    //             'id_stock_old'  => $idStockData[0]['id_stock'], // ID stok lama
    //             'id_stock_new'  => $newStockId, // ID stok baru
    //             'id_out_celup'  => $oldIdOC, // ID Oc
    //             'cluster_old'   => $idStockData[0]['nama_cluster'], // Cluster lama
    //             'cluster_new'   => $cluster, // Cluster baru
    //             'kgs'           => $kgsKirim, // Total kgs
    //             'cns'           => $cnsKirim, // Total cns
    //             'krg'           => $krgKirimOut, // Total krg
    //             'lot'           => $idStockData[0]['lot_stock'], // Lot stok lama
    //             'keterangan'    => "Pindah Order", // Keterangan pemindahan
    //             'admin'          => session()->get('username'), // Admin yang melakukan
    //             'created_at'    => date('Y-m-d H:i:s'), // Waktu pemindahan
    ...
    //     log_message('info', 'Total karung: ' . $totalKrgOut . ', detail: ' . json_encode($detailKirim[$i]));
    //     log_message('info', 'Total Kgs: ' . $totalKgs);
    //     log_message('info', 'Total Cns: ' . $totalCns);
    ...
    public function saveReturCelup()
    {
        ...
        $itemsRaw       = $this->request->getPost('items'); // JSON string
        $keterangan     = $this->request->getPost('keterangan');
        $idStock        = $this->request->getPost('id_stock');
        $namaCluster    = $this->request->getPost('nama_cluster');
        $kategori       = $this->request->getPost('kategori');
        ...
        if (!$itemsRaw) {
            return $this->response->setJSON(['success' => false, 'message' => 'Karung Tidak Terpilih']);
        }

        $items = json_decode($itemsRaw, true);
        if (!is_array($items) || empty($items)) {
            return $this->response->setJSON(['success' => false, 'message' => 'Format items tidak valid']);
        }

        $this->db->transBegin();

        try {
            // total untuk update stock
            $totalKgs = 0.0;
            $totalCns = 0;
            $totalKrg = 0;

            foreach ($items as $it) {
                // pastikan key ada dan tipe benar
                $idOut = $it['id_out_celup'];
                $lot   = isset($it['lot']) ? $it['lot'] : null;
                $kgsIt = isset($it['kgs']) ? (float)$it['kgs'] : 0.0;
                $cnsIt = isset($it['cns']) ? (int)$it['cns'] : 0;
                $krgIt = isset($it['krg']) ? (int)$it['krg'] : 0;

                // insert per karung dengan nilai per-item
                $this->historyStock->insert([
                ...
                if ($selectOutCelup) {
                    ...
                    $kgsKirim = $selectOutCelup['kgs_kirim'] ?? 0;
                    ...
                    $idBon = $selectOutCelup['id_bon'] ?? null;
                    if ($idBon) {
            ...
            // update stock sekali (pakai total dari items)
            $selectStock = $this->stockModel->where('id_stock', $idStock)->first();
            log_message('info', "Tot Kgs : $totalKgs, Stock Data: " . json_encode($selectStock));
            if ($selectStock) {
                ...
                // Kurangi dari stock_awal jika ada isinya
                if ($kgsStockAwal > 0) {

                    $this->stockModel->set('kgs_stock_awal', max(0, $kgsStockAwal - $totalKgs));
                    $this->stockModel->set('cns_stock_awal', max(0, $cnsStockAwal - $totalCns));
                    $this->stockModel->set('krg_stock_awal', max(0, $krgStockAwal - $totalKrg));
                }
                // Jika stock_awal kosong, kurangi dari in_out
                else {

                    $this->stockModel->set('kgs_in_out', max(0, $kgsInOut - $totalKgs));
                    $this->stockModel->set('cns_in_out', max(0, $cnsInOut - $totalCns));
                    $this->stockModel->set('krg_in_out', max(0, $krgInOut - $totalKrg));
                }

                $this->stockModel->where('id_stock', $idStock)->update();
            }
