...
class PemesananController extends BaseController
{
    ...
    public function selectClusterWarehouse(int $id)
    {
        ...
        foreach ($dataPemesanan as $key => $pem) {
            ...
            foreach ($getStyle as $i => $data) {
                // Ambil qty
                $urlQty = 'http://192.168.44.14/CapacityApps/public/api/getQtyOrder?no_model=' . urlencode($pem['no_model'])
        ...
        foreach ($dataRetur as $retur) {
            ...
            foreach ($getStyle as $i => $data) {
                // Ambil qty
                $urlQty = 'http://192.168.44.14/CapacityApps/public/api/getQtyOrder?no_model=' . $retur['no_model']
                    . '&style_size=' . urlencode($data['style_size'])
                    . '&area=' . $area;
    ...
    //                         $orderApiUrl = 'http://192.168.44.14/CapacityApps/public/api/getQtyOrder?no_model='
    ...
    public function sisaKebutuhanArea()
    {
        ...
        foreach ($dataPemesanan as $key => $pemesanan) {
            ...
            foreach ($getStyle as $i => $data) {
                // Ambil qty
                $urlQty = 'http://192.168.44.14/CapacityApps/public/api/getQtyOrder?no_model=' . urlencode($pemesanan['no_model'])
                    . '&style_size=' . urlencode($data['style_size'])
                    . '&area=' . $area;
    ...
    //         $url = sprintf(
    //             'http://192.168.44.14/CapacityApps/public/api/getQtyOrder?no_model=%s&style_size=%s&area=%s',
    //             $noModel,
