...
class ExcelController extends BaseController
{
    ...
    public function exportDatangBenang()
    {
        ...
        $sheet->mergeCells('A1:Y1'); // Menggabungkan sel untuk judul
        $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
        $sheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        ...
        foreach ($data as $index => $item) {
            ...
            if (isset($qtyMap[$uniqueKey])) {
                $item['kgs_material']  = $qtyMap[$uniqueKey]['kg_po'];
            } else {
                $item['kgs_material']  = 0;
            }
    ...
    public function exportScheduleNylon()
    {
        ...
        foreach ($dataPoPlus as $dtPoPlus) {
            ...
            $poPlusMap[$key] = $dtPoPlus['ttl_tambahan_kg'];
        }
        ...
        foreach ($data as &$dt) {
            ...
            if (strpos($dt->no_model, 'POGABUNGAN') === 0) {
                $cleanModel = str_replace('POGABUNGAN ', '', $dt->no_model);
                $models = explode('_', $cleanModel);
            } else {
                $models = [$dt->no_model];
            }
            ...
            $dt->total_kgs = implode(', ', $kgDetails);
            $dt->total_poplus = implode(', ', $kgPoPlusDetails);
            $dt->kgs_stock_awal = implode(', ', $stockAwalDetails);
            $dt->kgs_datang = implode(', ', $kgDatangDetails);
            $dt->tgl_datang = implode(', ', $tglDatangDetails);
    ...
    public function exportReportSisaPakaiNylon()
    {
        ...
        // Po Tambahan Gbn: 192.168.1.5Sub-header
        $sheet->mergeCells('R3:U3');
        $sheet->setCellValue('R3', 'PO TAMBAHAN GBN');
        $sheet->setCellValue('R4', 'TGL TERIMA PO(+) GBN');
        $sheet->setCellValue('S4', 'TGL PO(+) AREA');
    ...
    public function exportExcelJatahNoModel()
    {
        ...
        if ($noModel) {
            $apiUrl = 'http://192.168.1.5/CapacityApps/public/index.php/api/getQtyOrderPerArea?model=' . urlencode($noModel);
